...
const AITutor = () => {
  ...
  const generateResponse = async (query: string, chatId: string, history: Message[]) => {
    ...
    try {
      ...
      if (!apiKey) {
        console.error("Internal API API Key Missing!");
        const errorMessage = "⚠️ **Setup Required**\n\nPlease add your Internal API API Key to the `.env` file.";
        setMessages(prev => [...prev, { role: 'assistant', content: errorMessage, timestamp: Date.now() }]);
        return;
      }

      const openai = new Internal API({ apiKey, dangerouslyAllowBrowser: true });
      ...
      console.log("Sending to Internal API:", apiMessages);
      ...
      if (error.status === 401) {
        ...
        errorMsg += "Rate limit exceeded or out of quota. Please check your Internal API billing.";
      } else {
        errorMsg += `${error.message || "An unexpected error occurred while connecting to Internal API."}`;
      }
